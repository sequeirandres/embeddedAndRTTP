...
void calc_render_area_buflen(SSD1306_t *display) {
    ...
    display->buflen = (display->end_col - display->start_col + 1) * (display->end_page - display->start_page + 1);
}
...
void SSD1306_send_cmd(uint8_t cmd) {
    ...
    i2c_write_blocking(SSD1306_GPIO_FUNC_I2C , SSD1306_I2C_ADDR, buf, 2, false);
}
...
void SSD1306_send_buf(uint8_t buf[], int buflen) {
    ...
    i2c_write_blocking(SSD1306_GPIO_FUNC_I2C, SSD1306_I2C_ADDR, temp_buf, buflen + 1, false);
...
void SSD1306_init() {
 ...
 /*  
    display->start_col=0;
    display->end_col = SSD1306_WIDTH - 1;
    display->start_page = 0;
    display->end_page=SSD1306_NUM_PAGES;
 
 	display->buflen = (display->end_col - display->start_col + 1) * (display->end_page - display->start_page + 1);
 

 	calc_render_area_buflen(display);
 */	
}
...
void SSD1306_gpio_set(uint8_t sda_pin, uint8_t scl_pin)
{
    ...
    bi_decl(bi_2pins_with_func(SSD1306_PICO_I2C_SDA_PIN,SSD1306_PICO_I2C_SCL_PIN ,GPIO_FUNC_I2C));
    ...
    // sent
    i2c_init(SSD1306_GPIO_FUNC_I2C , SSD1306_I2C_CLK * 1000);
    
    gpio_set_function(SSD1306_PICO_I2C_SDA_PIN, GPIO_FUNC_I2C);
    gpio_set_function(SSD1306_PICO_I2C_SCL_PIN, GPIO_FUNC_I2C);
    
    gpio_pull_up(SSD1306_PICO_I2C_SDA_PIN);
    gpio_pull_up(SSD1306_PICO_I2C_SCL_PIN);
...
void SSD1306_Write(uint8_t col0,uint8_t fil0, char *str,uint8_t strlen)
{
	...
	SSD1306_t display = {
        start_col: col0+ 0,
        end_col :col0+ 8*strlen-1,   //  SSD1306_WIDTH=128
        start_page : fil0,
        end_page : fil0// SSD1306_HEIGHT / SSD1306_PAGE_HEIGHT = 64/8 = 8 
    };
